document.getElementById('processButtonMale').addEventListener('click', () => {
    const fileInput = document.getElementById('fileInputMale');
    ...
    reader.onload = (event) => {
        try {
            ...
            const formattedData = sourceData.map(item => {
                ...
                const tags = `${bodyFabric.replace(/\s+/g, '')},Male,${item['Description'] || ''}`;
                ...
                return {
                    ...
                    'Variant Metafield: mm-google-shopping.gender [single_line_text_field]': 'Male',
            ...
            const downloadLink = document.getElementById('downloadLinkMale');
...
document.getElementById('downloadLinkMale').style.display = 'none';
